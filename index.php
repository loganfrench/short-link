		...
		function createLink() {
			...
			$.ajax({
				...
				success: function(data) {
					...
					if(json.status == true) {
						$("#link").val("");
						addTable('<tr><td><a href="' + link + '" target="_blank">' + link + '</a></td><td><a href="http://<?=$_SERVER['SERVER_NAME'].'/'?>' + json.link + '" target="_blank"><?=$_SERVER['SERVER_NAME'].'/'?>' + json.link + '</a></td><td>0</td><td>' + json.created + '</td></tr>');
					}
		...
		$.ajax({
			...
			success: function(data) {
				...
				for(var i = 0; i < json.length; i++) {
					addTable('<tr><td><a href="' + json[i].link + '" target="_blank">' + json[i].link + '</a></td><td><a href="http://<?=$_SERVER['SERVER_NAME'].'/'?>' + json[i].key + '" target="_blank"><?=$_SERVER['SERVER_NAME'].'/'?>' + json[i].key + '</a></td><td>' + json[i].views + '</td><td>' + json[i].created + '</td></tr>');
				}
			}
		});
	</script>
